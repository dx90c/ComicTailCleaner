# ======================================================================
# 檔案名稱：ComicTailCleaner_v12.4.0.py
# 版本號：12.4.0
...
#
# === 12.4.0 版本更新內容 ===
# - 【性能優化】再次重構了核心比對算法，採用更高效的數據結構進行操作，
#   解決了 v12.3.x 版本中因數據處理不當導致的比對速度變慢問題。
# - 【UI 優化】主控制按鈕佈局固定為 [開始執行] [暫停/恢復] [終止]，
#   通過啟用/禁用狀態進行管理，交互邏輯更清晰，避免誤觸。
# - 【功能修正】徹底修復了「暫停」功能在多進程計算階段實際無效的問題，
#   現在點擊暫停會確實阻塞後台任務，並正確更新狀態欄文字。
# - 【健壯性】增強了程式的錯誤處理和執行緒控制邏輯。
#
# === 12.3.9 版本更新內容 ===
# - 【性能優化】重構了「互相比對」模式的算法，採用預分組策略提升速度。
# - 【UI 優化】調整了主視窗的按鈕佈局。
# ======================================================================
...
def load_ad_hashes(ad_folder_path, rebuild_cache=False):
    ...
    ad_images = [os.path.join(r, f) for r, _, fs in os.walk(ad_folder_path) if f.lower().endswith(('.png','.jpg','.jpeg'))]
...
# === 8. 核心工具函數 (續) ===
def get_all_subfolders(root_folder, excluded_folders=None, progress_queue=None, control_events=None, time_filter_config=None):
    # ... 此函數無變更 ...
    all_subfolders = []
...
def extract_last_n_files_from_folders(folder_paths, count, enable_limit, progress_queue=None, control_events=None):
    # ... 此函數無變更 ...
...
class ImageComparisonEngine:
    def __init__(self, config, progress_queue=None, control_events=None):
        ...
        print(f"ImageComparisonEngine (v12.4.0) initialized.", flush=True)

    def _check_control(self):
        if self.control_events:
            if self.control_events['cancel'].is_set(): return True
            if self.control_events['pause'].is_set():
                self._update_progress(text="任務已暫停..."); self.control_events['pause'].wait()
                if self.control_events['cancel'].is_set(): return True
    ...
    def _calculate_hashes_with_cache(self, file_paths, cache_manager, description=""):
        ...
        if paths_to_recalc:
            ...
            with Pool(processes=cpu_count()) as pool:
                results = pool.imap_unordered(_pool_worker_process_image, paths_to_recalc)
                total_to_calc = len(paths_to_recalc)
                for i, (path, data) in enumerate(results):
                    # 【v12.4.0 修正】在迴圈內部檢查暫停/取消
                    if self._check_control(): pool.terminate(); return None
                    if data: file_data[path] = data; cache_manager.update_data(path, data)
                    progress = int((i + 1) / total_to_calc * 100)
                    self._update_progress(p_type='progress', value=progress, text=f"計算哈希中...({i+1}/{total_to_calc})")
    ...
    def _find_similar_images(self):
        ...
        if self.config['comparison_mode'] == 'ad_comparison':
            ...
            for h, paths in hash_groups.items():
                if len(paths) > 1:
                    base_path = paths[0]
                    for other_path in paths[1:]: found_duplicates.append((base_path, other_path, 100.0))
                ...
                for j in range(i + 1, n):
                    ...
                    if hash1 - hash2 <= max_diff:
                        ...
                        for path1 in hash_groups[hash1]:
                            for path2 in hash_groups[hash2]: found_duplicates.append((path1, path2, sim))
    ...
    def _detect_qr_codes(self): return [], {}
...
class SettingsGUI(tk.Toplevel):
    ...
    def _setup_bindings(self): self.comparison_mode_var.trace_add("write", self._toggle_ad_folder_entry_state); self.enable_time_filter_var.trace_add("write", self._toggle_time_filter_fields)
    def _browse_folder(self, entry):
        folder = filedialog.askdirectory(parent=self);
        if folder: entry.delete(0, tk.END); entry.insert(0, folder)
    def _update_threshold_label(self, val): self.threshold_label.config(text=f"{float(val):.0f}%")
    def _toggle_ad_folder_entry_state(self, *args): self.ad_folder_entry.config(state=tk.NORMAL if self.comparison_mode_var.get() == "ad_comparison" else tk.DISABLED)
...
class MainWindow(tk.Tk):
    ...
    def _reset_control_buttons(self, final_status_text="任務完成"):
    ...
    def _on_closing(self):
        if messagebox.askokcancel("關閉程式", "確定要關閉程式嗎？"):
            self.cancel_event.set() # Ensure thread terminates
            self.destroy()
