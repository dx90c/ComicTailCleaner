...
class FolderCreationCacheManager:
    ...
    def invalidate_cache(self) -> None:
        ...
        if os.path.exists(self.cache_file_path):
            try: os.remove(self.cache_file_path)
            except OSError as e: log_error(f"刪除資料夾建立時間快取檔案時發生錯誤: {e}", True)
    ...
    def _find_similar_images(self, target_files: list[str], scan_cache_manager: ScannedImageCacheManager) -> tuple[list, dict]:
        ...
        if self.config['comparison_mode'] == 'ad_comparison':
            ...
            for i, (target_path, target_data) in enumerate(target_file_data.items()):
                ...
                if (i + 1) % 200 == 0 or (i + 1) == total_comparisons: self._update_progress(p_type='progress', value=int((i+1)/total_comparisons*100), text=f"廣告比對中... ({i+1}/{total_comparisons})")
                    ...
                    if hash1 - hash2 <= max_diff:
                        ...
                        for path1 in hash_groups[hash1]:
                            for path2 in hash_groups[hash2]: 
                                found_items.append((min(path1, path2), max(path1, path2), f"{sim:.1f}%"))
                if (i + 1) % 100 == 0 or (i + 1) == n: self._update_progress(p_type='progress', value=int((i+1)/n*100), text=f"唯一哈希互相比對中... ({i+1}/{n})")
...
class SettingsGUI(tk.Toplevel):
    ...
    def _create_widgets(self, frame: ttk.Frame) -> None:
        ...
        self.rebuild_scan_cache_var = tk.BooleanVar(); ttk.Checkbutton(cache_time_frame, text="重建掃描圖片快取", variable=self.rebuild_scan_cache_var).pack(anchor="w")
        self.rebuild_folder_cache_var = tk.BooleanVar(); ttk.Checkbutton(cache_time_frame, text="重建資料夾時間快取", variable=self.rebuild_folder_cache_var).pack(anchor="w")
    ...
    def _load_settings_into_gui(self) -> None:
        ...
        self.similarity_threshold_var.set(self.config.get('similarity_threshold', 98.0)); self._update_threshold_label(self.config.get('similarity_threshold', 98.0))
    ...
    def _save_settings(self) -> bool:
        try:
            ...
            self.master.config.update({'rebuild_scan_cache': False, 'rebuild_folder_cache': False}); return True
        except ValueError: messagebox.showerror("錯誤", "提取數量必須是有效的數字。", parent=self); return False
...
class MainWindow(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.all_found_items, self.all_file_data = [], {}; self.selected_files, self.banned_groups = set(), set()
    ...
    def check_queue(self) -> None:
        try:
            while True:
                ...
                elif msg['type'] == 'result': self.all_found_items, self.all_file_data = msg.get('data', []), msg.get('meta', {}); self._populate_treeview()
                elif msg['type'] == 'finish':
                    self._reset_control_buttons(msg.get('text', '任務完成'))
                    if not self.all_found_items and "取消" not in msg.get('text', ''): messagebox.showinfo("掃描結果", "未找到符合條件的相似或廣告圖片。")
        except Empty: pass
    ...
    def _run_scan_in_thread(self) -> None:
        ...
        try:
            ...
            self.progress_queue.put({'type': 'result', 'data': found_items, 'meta': all_file_data})
            unique_targets = len(set(p[1] for p in found_items))
            final_text = f"掃描完成。找到 {unique_targets} 個不重複的目標。" if self.config['comparison_mode'] != 'qr_detection' else f"掃描完成。共找到 {len(found_items)} 個目標。"
            self.progress_queue.put({'type': 'finish', 'text': final_text})
        except Exception as e:
    ...
    def _load_and_display_selected_image(self) -> None:
        ...
        if not selected:
            ...
            for label in [self.target_path_label, self.compare_path_label]: label.config(text="")
            return
        item_id = selected[0]; tags = self.tree.item(item_id, "tags")
        ...
        self.pil_img_target = self._load_pil_image(sel_path, self.target_path_label)
    ...
    def _load_pil_image(self, path: str | None, label_widget: tk.Label) -> Image.Image | None:
        ...
        try:
            with Image.open(path) as img:
                ...
                label_widget.config(text=f"路徑: {path}"); return img.copy()
        except Exception as e:
    ...
    def _navigate_image(self, event: tk.Event, direction: str) -> str:
        ...
        if direction == "Down":
            ...
            parent = self.tree.parent(current_id)
            if parent != "" and current_id == self.tree.get_children(parent)[0]:
                target_id = parent
            else:
        ...
        if target_id:
            self.tree.selection_set(target_id); self.tree.focus(target_id); self.tree.see(target_id)
        return "break"
        ...
        if 'qr_item' in tags:
            ...
            for child_id in self.tree.get_children(item_id):
                if 'protected_item' not in self.tree.item(child_id, "tags"):
                    self._update_child_selection(child_id, select)
            ...
            if (parent_id := self.tree.parent(item_id)):
    ...
    def _ban_group(self) -> None:
        ...
        if group_key: self.banned_groups.add(group_key); self._populate_treeview()
    def _unban_all_groups(self) -> None: self.banned_groups.clear(); self._populate_treeview()
