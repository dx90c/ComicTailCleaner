...
class MainWindow(tk.Tk):
    ...
    def _create_treeview(self, parent_frame: ttk.Frame):
        ...
        widths={"status":40,"filename":300,"path":300,"count":50,"size":100,"ctime":130,"similarity":100}
...
###
    ...
    def _mark_new_ads(self) -> None:
        paths_to_select = set()
        for item_id in self.tree.get_children(""):
             if self.item_to_path.get(item_id) and "QR Code" in self.tree.item(item_id, "values")[6]:
                 paths_to_select.add(self.item_to_path[item_id])
        if not paths_to_select: messagebox.showinfo("提示", "目前沒有可自動標記的新廣告圖。", parent=self); return
        self.selected_files.update(paths_to_select)
        self._refresh_all_checkboxes()
    ...
    def _copy_selected_to_ad_library(self):
        ...
        ad_cache = ScannedImageCacheManager(ad_dir, ad_dir, 'ad_comparison')
        copied = 0
