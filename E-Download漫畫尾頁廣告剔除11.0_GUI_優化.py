...
def extract_last_n_files_from_folders(folder_paths, count):
    """
    從每個資料夾中提取最後 N 個圖片檔案的路徑。
    Args:
        folder_paths (list): 資料夾路徑列表。
        count (int): 要提取的最後 N 個檔案數量。
    Returns:
        dict: 每個資料夾路徑到其最後 N 個圖片檔案路徑列表的映射。
    """
    extracted_files = {}
    ...
    for folder_path in folder_paths:
        ...
        try:
            ...
            # Extract paths of the last N files
            extracted_files[folder_path] = [item[0] for item in image_files[-count:]]
        except PermissionError:
            log_error(f"無權限訪問資料夾中的檔案: {folder_path}", include_traceback=False) # Added include_traceback=False
        except Exception as e:
...
class ImageComparisonEngine:
    def __init__(self, root_scan_folder, ad_folder_path, extract_count, excluded_folders,
                 ...
                 scanned_hashes_cache_manager): # New scanned_hashes_cache_manager parameter
    ...
    def generate_extracted_files(self, folder_creation_cache_manager):
        ...
        extracted_files_dict = extract_last_n_files_from_folders(all_folders, self.extract_count)
...
class SettingsGUI:
    ...
    def _create_widgets(self, frame):
        ...
        ttk.Label(basic_settings_frame, text="提取末尾圖片數量:").grid(row=0, column=0, sticky="w", pady=2)
        self.extract_count_var = tk.StringVar()
        ttk.Spinbox(basic_settings_frame, from_=1, to=100, textvariable=self.extract_count_var, width=5).grid(row=0, column=1, sticky="w", padx=5)
        ttk.Label(basic_settings_frame, text="(以檔名為判斷基準，從每個資料夾末尾提取N張圖片進行比對)").grid(row=0, column=2, sticky="w", padx=5)

        ttk.Label(basic_settings_frame, text="相似度閾值 (%):").grid(row=1, column=0, sticky="w", pady=2)
        ...
        ttk.Scale(basic_settings_frame, from_=50, to=100, orient="horizontal",
                  ...
                  command=self._update_threshold_label).grid(row=1, column=1, sticky="w", padx=5)
        self.threshold_label = ttk.Label(basic_settings_frame, text="")
        self.threshold_label.grid(row=1, column=2, sticky="w", padx=5)

        ttk.Label(basic_settings_frame, text="排除資料夾名稱 (逗號分隔):").grid(row=2, column=0, sticky="w", pady=2)
        self.excluded_folders_text = tk.Text(basic_settings_frame, width=40, height=4)
        self.excluded_folders_text.grid(row=2, column=1, columnspan=2, sticky="ew", padx=5)
    ...
    def _save_settings(self):
        ...
        try:
            ...
            messagebox.showerror("輸入錯誤", "抽取檔案數量或相似度閾值必須是有效數字！")
            return False
        except Exception as e:
...
class MainWindow:
    ...
    def _create_widgets(self):
        ...
        self.target_image_frame.grid_rowconfigure(0, weight=1)
        self.target_image_frame.grid_columnconfigure(0, weight=1)
        ...
        self.compare_image_frame.grid_rowconfigure(0, weight=1)
        self.compare_image_frame.grid_columnconfigure(0, weight=1)
