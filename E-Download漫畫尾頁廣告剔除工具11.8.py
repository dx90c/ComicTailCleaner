# 起一 https://gemini.google.com/app/226d410ffc339764
# 起二 https://gemini.google.com/app/eca455c0a5ec54ce
...
# -----------------------------------------------------------------------------
# 腳本版本號，便於追蹤更新
SCRIPT_VERSION = "11.8" # 版本號更新為 11.8
# 腳本名稱
SCRIPT_NAME = "E-Download漫畫尾頁廣告剔除工具"
# 版本備註
VERSION_NOTE = "新增：只掃描每個資料夾的最後N頁功能" 
...
def update_image_hashes_cache(image_paths_to_hash, cache_file_path, hash_size, progress_callback=None):
    """
    掃描指定圖片路徑列表中的所有圖片，更新其哈希值、尺寸和修改時間到 JSON 緩存。
    支援增量更新：只計算新圖片或已修改圖片的哈希，大幅減少重複計算。
    新增 progress_callback 參數用於向 GUI 更新進度。
    """
    ...
    log_message(f"預計處理 {total_images} 張圖片。")

    for image_path in image_paths_to_hash:
        processed_count += 1
        if progress_callback:
            progress_callback(f"計算圖片哈希: {processed_count}/{total_images}")

        if not os.path.exists(image_path):
            # log_message(f"圖片文件 '{image_path}' 不存在，將從緩存中移除（如果存在）。", "warning") # 避免過多輸出
            continue
...
def update_ad_image_hashes_cache(ad_folders, cache_file_path, hash_size, progress_callback=None):
    """
    ...
    log_message(f"預計處理 {total_images} 張廣告圖片。")

    for image_path in all_ad_image_paths:
        processed_count += 1
        if progress_callback:
            progress_callback(f"計算廣告哈希: {processed_count}/{total_images}")
...
class SettingsWindow(tk.Toplevel):
    ...
    def update_image_hash_cache_gui(self):
        ...
        def run_update():
            try:
                ...
                self.master.update_status("正在更新圖片庫哈希緩存...")
                # 傳遞 image_paths_to_hash 給 update_image_hashes_cache
                update_image_hashes_cache(image_paths_to_hash, IMAGE_HASH_CACHE_FILE, current_hash_size, 
                                        progress_callback=self.master.update_status)
    ...
    def update_ad_hash_cache_gui(self):
        ...
        def run_update():
            try:
                current_hash_size = int(self.hash_size_entry.get())
                self.master.update_status("正在更新廣告圖片哈希緩存...")
                # 傳遞 progress_callback 以便在線程中更新 GUI 狀態
    ...
    def _run_analysis_logic(self):
        ...
        try:
            ...
            if self.app_config.use_image_hash_cache:
                self.after(0, lambda: self.update_status("正在更新/載入圖片哈希緩存..."))
                ...
                self.after(0, lambda: self.update_status("未啟用圖片哈希緩存，正在實時計算圖片哈希..."))
                ...
                for img_path in all_image_paths:
                    processed_count += 1
                    self.after(0, lambda msg=f"實時計算哈希: {processed_count}/{total_images}": self.update_status(msg))
            ...
            if self.app_config.compare_mode == "ad": # 只有在廣告比對模式才需要廣告哈希
                self.after(0, lambda: self.update_status("正在更新/載入廣告圖片哈希緩存..."))
                # 這裡調用 update_ad_image_hashes_cache，現在它會被正確找到
