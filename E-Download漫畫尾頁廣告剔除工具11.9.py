...
# -----------------------------------------------------------------------------
# 腳本版本號，便於追蹤更新
SCRIPT_VERSION = "11.9" # 版本號更新為 11.9
# 腳本名稱
SCRIPT_NAME = "E-Download漫畫尾頁廣告剔除工具"
# 版本備註
VERSION_NOTE = "新增：只掃描每個資料夾的最後N頁功能；優化：狀態訊息顯示；改進：圖片哈希緩存變為全域緩存，狀態訊息精細化" 
...
def update_image_hashes_cache(image_paths_to_process, cache_file_path, hash_size):
    """
    更新圖片的感知哈希緩存。這個緩存現在是一個全域緩存。
    它會：
    1. 從緩存中移除所有對應文件已不存在的哈希條目。
    2. 對於 `image_paths_to_process` 中的圖片：
       - 如果已存在於緩存且修改時間未變，則跳過。
       - 否則，計算哈希值並更新或添加至緩存。

    參數：
    - image_paths_to_process: 列表，本次操作需要檢查/處理的圖片路徑（通常是根據用戶篩選條件得出的子集）。
    - cache_file_path: 圖片哈希緩存文件的路徑。
    - hash_size: 感知哈希計算的哈希大小。
    返回: tuple (global_cache, calculated_count, loaded_from_cache_count)
    """
    log_message(f"開始更新全域圖片哈希緩存：{cache_file_path}")
    global_cache = load_json(cache_file_path) # 載入現有全域緩存

    initial_cached_count = len(global_cache)
    log_message(f"緩存中已存在 {initial_cached_count} 張圖片的哈希。")

    # Phase 1: 清理緩存中已不存在的圖片路徑
    paths_to_remove = [path for path in global_cache if not os.path.exists(path)]
    for path in paths_to_remove:
        del global_cache[path]
        log_message(f"從全域圖片哈希緩存中移除不存在的圖片：'{os.path.basename(path)}'")
    
    log_message(f"清理後，緩存中剩餘 {len(global_cache)} 張圖片的哈希。")

    # Phase 2: 對當前需要處理的圖片進行檢查和哈希計算
    # 這裡的 image_paths_to_process 是當前掃描條件篩選出的圖片子集
    log_message(f"本次掃描將處理 {len(image_paths_to_process)} 張圖片。")
    
    calculated_count = 0
    loaded_from_cache_count = 0

    for image_path in image_paths_to_process:
        # 如果文件不存在，則跳過 (雖然在清理階段已經處理，但這裡再次檢查以防萬一)
        if not os.path.exists(image_path):
            continue

        file_mod_time_hash, file_mod_time_formatted = get_file_modification_time_and_hash(image_path)
        if file_mod_time_hash is None:
            continue

        # 檢查該圖片是否已存在於全域緩存中且是最新的
        if image_path in global_cache and \
           global_cache[image_path].get("mod_time_hash") == file_mod_time_hash:
            loaded_from_cache_count += 1
        else:
            # 圖片是新的，或者已被修改，需要重新計算哈希
            image_hash, dimensions = calculate_image_hash_and_dimensions(image_path, hash_size)
            if image_hash:
                global_cache[image_path] = {
                    "hash": image_hash,
                ...
                calculated_count += 1
                # log_message(f"計算並儲存圖片 '{os.path.basename(image_path)}' 的哈希。") # 避免過多日誌
            else:
                log_message(f"警告：無法計算圖片 '{image_path}' 的哈希值，將跳過。", "warning")

    save_json(global_cache, cache_file_path) # 保存更新後的緩存
    log_message(f"全域圖片哈希緩存更新完成。目前緩存中共有 {len(global_cache)} 張圖片的哈希。")
    return global_cache, calculated_count, loaded_from_cache_count
...
class SettingsWindow(tk.Toplevel):
    ...
    def update_image_hash_cache_gui(self):
        ...
        def run_update():
            try:
                self.master.update_status("正在獲取所有圖片路徑...")
                # 這裡需要獲取所有符合篩選條件（包括scan_last_n_pages）的圖片路徑
                # 對於手動更新，我們假設用戶可能希望更新所有漫畫庫中的圖片哈希，
                # 而不是只更新當前篩選條件下的子集，這樣才能作為一個「全域更新」功能。
                # 因此，這裡會無視 scan_last_n_pages，獲取所有圖片。
                # 如果用戶只需要更新部分，可以通過「開始執行」來觸發。
                all_paths_for_full_update = []
                if self.scan_recursive_var.get():
                     # 如果是遞歸掃描，則直接從 root_folder 開始遍歷
                    all_paths_for_full_update = get_image_paths_in_folder(root_folder, recursive=True)
                else:
                    all_paths_for_full_update = get_image_paths_in_folder(root_folder, recursive=False)


                if not all_paths_for_full_update:
                ...
                # 調用 update_image_hashes_cache，並接收返回的計數
                _, calculated_count, loaded_count = update_image_hashes_cache(all_paths_for_full_update, IMAGE_HASH_CACHE_FILE, current_hash_size)

                # 根據計數更新狀態訊息
                if calculated_count > 0 and loaded_count > 0:
                    status_message = "圖片哈希緩存已更新完成 (部分計算, 部分載入)。"
                elif calculated_count > 0 and loaded_count == 0:
                    status_message = "圖片哈希緩存已更新完成 (全部計算)。"
                elif calculated_count == 0 and loaded_count > 0:
                    status_message = "圖片哈希緩存已更新完成 (全部載入)。"
                else:
                    status_message = "圖片哈希緩存已更新完成 (無變更)。"

                self.master.after(0, lambda: messagebox.showinfo("緩存更新", status_message))
                self.master.update_status("準備就緒...")
            except ValueError:
    ...
    def update_ad_hash_cache_gui(self):
        ...
        def run_update():
            try:
                current_hash_size = int(self.hash_size_entry.get())
                self.master.update_status("正在更新廣告圖片哈希緩存 - 計算中...")
...
class MainApplication(tk.Tk):
    ...
    def _run_analysis_logic(self):
        ...
        try:
            ...
            # 這裡的 all_image_paths 是需要被當前掃描邏輯「考慮」的圖片列表
            # 這個列表會傳遞給 update_image_hashes_cache，由後者決定是否重新計算哈希
            all_image_paths = []
            ...
            if not all_image_paths:
                ...
                return
            log_message(f"最終確定了 {len(all_image_paths)} 張圖片作為當前比對範圍。")


            # --- 3. 處理圖片哈希緩存 (全域緩存邏輯) ---
            # all_image_data 將會是全域緩存的內容
            all_image_data = {} 
            if self.app_config.use_image_hash_cache:
                self.after(0, lambda: self.update_status("正在處理圖片哈希緩存...")) # 統一為處理中
                # 將 all_image_paths 傳遞給 update_image_hashes_cache，它會據此決定要計算哪些哈希
                all_image_data, calculated_count, loaded_count = update_image_hashes_cache(
                    all_image_paths, IMAGE_HASH_CACHE_FILE, self.app_config.hash_size
                )
                
                # 根據計算和載入的數量來設定更精確的狀態訊息
                if calculated_count > 0 and loaded_count > 0:
                    self.after(0, lambda: self.update_status("正在載入並計算圖片哈希..."))
                elif calculated_count > 0:
                    self.after(0, lambda: self.update_status("正在計算圖片哈希..."))
                elif loaded_count > 0:
                    self.after(0, lambda: self.update_status("正在載入圖片哈希..."))
                else:
                    self.after(0, lambda: self.update_status("沒有圖片哈希需要處理。"))


                if not all_image_data:
                    self.after(0, lambda: messagebox.showinfo("提示", "圖片緩存為空或所有圖片均無法處理。"))
                    self.after(0, lambda: self.update_status("圖片緩存問題。"))
                    return
                log_message(f"從全域緩存中獲取了 {len(all_image_data)} 張圖片的哈希數據。")
            else:
                self.after(0, lambda: self.update_status("正在計算圖片哈希..."))
                
                # 實時計算哈希，並只保留當前掃描範圍的圖片哈希
                for img_path in all_image_paths: # 只計算當前範圍內的圖片
                ...
                log_message(f"實時計算完成，共 {len(all_image_data)} 張圖片的哈希、尺寸和修改時間 (僅限當前掃描範圍)。")


            # --- 4. 處理廣告圖片哈希緩存 ---
            ad_hashes_map = {} # 儲存廣告圖片的哈希到路徑的映射
            if self.app_config.compare_mode == "ad": # 只有在廣告比對模式才需要廣告哈希
                self.after(0, lambda: self.update_status("正在計算廣告圖片哈希...")) 
                ...
                if not ad_hashes_map:
                    ...
                    return
                log_message(f"廣告哈希緩存中共有 {len(ad_hashes_map)} 個廣告哈希。")
            ...
            # 再次過濾 all_image_data，這次是為了比對邏輯
            # 注意：這裡的 all_image_data 已經包含了所有曾經掃描過的圖片，但我們只對當前掃描範圍的圖片進行比對
            # 所以需要根據 all_image_paths 來篩選
            filtered_image_data_for_comparison = {}
            for path in all_image_paths: # 只對當前掃描範圍內的圖片進行最終過濾和比對
                if path in all_image_data: # 確保該圖片在哈希數據中存在
                    data = all_image_data[path]
                    if data and data.get("hash") and data["hash"] not in excluded_hashes_set:
                        ...
                        filtered_image_data_for_comparison[path] = data
            log_message(f"排除過濾後，將有 {len(filtered_image_data_for_comparison)} 張圖片用於本次比對。")
            ...
            if self.app_config.compare_mode == "ad":
                self.found_images_data = find_similar_images_with_ads(
                    filtered_image_data_for_comparison, ad_hashes_map, self.app_config.threshold, self.app_config.hash_size
                )
            elif self.app_config.compare_mode == "self":
                self.found_images_data = find_similar_images_within_folders(
                    filtered_image_data_for_comparison, self.app_config.threshold, self.app_config.hash_size
                )
            elif self.app_config.compare_mode == "qr":
                self.found_images_data = scan_qr_codes(filtered_image_data_for_comparison)
